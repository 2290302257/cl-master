...
namespace kcp2k
{
    public static class Common
    {
        ...
        public static bool ResolveHostname(string hostname, out IPAddress[] addresses)
        {
            ...
            {
                KcpLog.Info?.Invoke($"[KCP] Failed to resolve host: {hostname} reason: {exception}");
                addresses = null;
                return false;
            }
        }
        ...
        public static void ConfigureSocketBuffers(Socket socket, int recvBufferSize, int sendBufferSize)
        {
            ...
            {
                KcpLog.Warning?.Invoke($"[KCP] failed to set Socket RecvBufSize = {recvBufferSize} SendBufSize = {sendBufferSize}");
            }


            KcpLog.Info?.Invoke($"[KCP] RecvBuf = {initialReceive}=>{socket.ReceiveBufferSize} ({socket.ReceiveBufferSize/initialReceive}x) SendBuf = {initialSend}=>{socket.SendBufferSize} ({socket.SendBufferSize/initialSend}x)");
        }
