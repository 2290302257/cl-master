    ...
    public class KcpServer
    {
        ...
        static Socket CreateServerSocket(bool DualMode, ushort port)
        {
            if (DualMode)
            {
                ...
                {
                    KcpLog.Warning?.Invoke($"[KCP] Failed to set Dual Mode, continuing with IPv6 without Dual Mode. Error: {e}");
                }
        ...
        public virtual void Start(ushort port)
        {
            ...
            if (socket != null)
            {
                KcpLog.Warning?.Invoke("[KCP] Server: already started!");
                return;
            }
        ...
        protected virtual bool RawReceiveFrom(out ArraySegment<byte> segment, out int connectionId)
        {
            ...
            catch (SocketException e)
            {
                ...
                KcpLog.Info?.Invoke($"[KCP] Server: ReceiveFrom failed: {e}");
            }
        ...
        protected virtual void RawSend(int connectionId, ArraySegment<byte> data)
        {
            ...
            if (!connections.TryGetValue(connectionId, out KcpServerConnection connection))
            {
                KcpLog.Warning?.Invoke($"[KCP] Server: RawSend invalid connectionId={connectionId}");
                return;
            }
            ...
            {
                KcpLog.Error?.Invoke($"[KCP] Server: SendTo failed: {e}");
            }
        }
        ...
        protected virtual KcpServerConnection CreateConnection(int connectionId)
        {
            ...
            void OnConnectedCallback(KcpServerConnection conn)
            {
                ...
                KcpLog.Info?.Invoke($"[KCP] Server: added connection({connectionId})");
                ...
                KcpLog.Info?.Invoke($"[KCP] Server: OnConnected({connectionId})");
            ...
            void OnDisconnectedCallback()
            {
                ...
                // call mirror event
                KcpLog.Info?.Invoke($"[KCP] Server: OnDisconnected({connectionId})");
