    ...
    public class KcpServerConnection : KcpPeer
    {
        ...
        public void RawInput(ArraySegment<byte> segment)
        {
            ...
            if (state == KcpState.Authenticated)
            {
                ...
                {
                    KcpLog.Warning?.Invoke($"[KCP] ServerConnection: dropped message with invalid cookie: {messageCookie} expected: {cookie} state: {state}");
                    return;
                }
            }
            ...
            switch (channel)
            {
                ...
                default:
                {
                    ...
                    KcpLog.Warning?.Invoke($"[KCP] ServerConnection: invalid channel header: {channel}, likely internet noise");
                    break;
                }
            }
        }
