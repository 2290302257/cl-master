...
namespace kcp2k
{
    public abstract class KcpPeer
    {
        ...
        protected KcpPeer(KcpConfig config, uint cookie)
        {
            ...
            this.cookie = cookie;
            KcpLog.Info?.Invoke($"[KCP] {GetType()}: created with cookie={cookie}");
            ...
            if (ReceiveNextReliable(out KcpHeaderReliable header, out ArraySegment<byte> message))
            {
                ...
                switch (header)
                {
                    case KcpHeaderReliable.Hello:
                    {
                        ...
                        KcpLog.Info?.Invoke($"[KCP] {GetType()}: received hello with cookie={cookie}");
            ...
            while (ReceiveNextReliable(out KcpHeaderReliable header, out ArraySegment<byte> message))
            {
                ...
                switch (header)
                {
                    case KcpHeaderReliable.Hello:
                    {
                        ...
                        // GetType() shows Server/ClientConn instead of just Connection.
                        KcpLog.Warning?.Invoke($"{GetType()}: received invalid header {header} while Authenticated. Disconnecting the connection.");
                        Disconnect();
                        break;
                    }
                    case KcpHeaderReliable.Data:
        ...
        protected void OnRawInputReliable(ArraySegment<byte> message)
        {
            ...
            {
                // GetType() shows Server/ClientConn instead of just Connection.
                KcpLog.Warning?.Invoke($"[KCP] {GetType()}: Input failed with error={input} for buffer with length={message.Count - 1}");
            }
        }
        ...
        protected void OnRawInputUnreliable(ArraySegment<byte> message)
        {
            ...
            switch (header)
            {
                ...
                {
                    // GetType() shows Server/ClientConn instead of just Connection.
                    KcpLog.Info?.Invoke($"[KCP] {GetType()}: received disconnect message");
                    Disconnect();
                    break;
                }
            }
        ...
        void SendUnreliable(KcpHeaderUnreliable header, ArraySegment<byte> content)
        {
            ...
            if (content.Count > unreliableMax)
            {
                // otherwise content is larger than MaxMessageSize. let user know!
                // GetType() shows Server/ClientConn instead of just Connection.
                KcpLog.Error?.Invoke($"[KCP] {GetType()}: Failed to send unreliable message of size {content.Count} because it's larger than UnreliableMaxMessageSize={unreliableMax}");
                return;
            }
        ...
        public void SendHello()
        {
            ...
            // GetType() shows Server/ClientConn instead of just Connection.
            KcpLog.Info?.Invoke($"[KCP] {GetType()}: sending handshake to other end with cookie={cookie}");
        ...
        public virtual void Disconnect()
        {
            ...
            // GetType() shows Server/ClientConn instead of just Connection.
            KcpLog.Info?.Invoke($"[KCP] {GetType()}: Disconnected.");
