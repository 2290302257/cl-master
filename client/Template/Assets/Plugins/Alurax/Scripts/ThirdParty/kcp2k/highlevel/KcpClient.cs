...
namespace kcp2k
{
    public class KcpClient : KcpPeer
    {
        ...
        {
            KcpLog.Info?.Invoke($"[KCP] Client: OnConnected");
            connected = true;
            OnConnectedCallback();
        }
        ...
        {
            KcpLog.Info?.Invoke($"[KCP] Client: OnDisconnected");
            connected = false;
            socket?.Close();
            socket = null;
            remoteEndPoint = null;
        ...
        public void Connect(string address, ushort port)
        {
            if (connected)
            {
                KcpLog.Warning?.Invoke("[KCP] Client: already connected!");
                return;
            }
            ...
            KcpLog.Info?.Invoke($"[KCP] Client: connect to {address}:{port}");
        ...
        protected virtual bool RawReceive(out ArraySegment<byte> segment)
        {
            ...
            catch (SocketException e)
            {
                ...
                KcpLog.Info?.Invoke($"[KCP] Client.RawReceive: looks like the other end has closed the connection. This is fine: {e}");
                base.Disconnect();
                return false;
            }
        }
        ...
        protected override void RawSend(ArraySegment<byte> data)
        {
            ...
            catch (SocketException e)
            {
                ...
                // but connections should never just end silently.
                // at least log a message for easier debugging.
                KcpLog.Info?.Invoke($"[KCP] Client.RawSend: looks like the other end has closed the connection. This is fine: {e}");
        ...
        public void Send(ArraySegment<byte> segment, KcpChannel channel)
        {
            if (!connected)
            {
                KcpLog.Warning?.Invoke("[KCP] Client: can't send because not connected!");
                return;
            }
        ...
        public void RawInput(ArraySegment<byte> segment)
        {
            ...
            if (messageCookie == 0)
            {
                KcpLog.Error?.Invoke($"[KCP] Client: received message with cookie=0, this should never happen. Server should always include the security cookie.");
            }

            if (cookie == 0)
            {
                cookie = messageCookie;
                KcpLog.Info?.Invoke($"[KCP] Client: received initial cookie: {cookie}");
            }
            else if (cookie != messageCookie)
            {
                KcpLog.Warning?.Invoke($"[KCP] Client: dropping message with mismatching cookie: {messageCookie} expected: {cookie}.");
                return;
            }
            ...
            switch (channel)
            {
                ...
                default:
                {
                    ...
                    KcpLog.Warning?.Invoke($"[KCP] Client: invalid channel header: {channel}, likely internet noise");
                    break;
                }
            }
        }
